...
describe("IPFS Client", () => {
  ...
  describe("Error Handling", () => {
    test("should throw error for invalid CID", async () => {
      await expect(client.retrievePackage("invalid-cid")).rejects.toThrow(
        "Failed to retrieve package from IPFS",
      );
    });

    test("should throw error for invalid encrypted package CID", async () => {
      await expect(
        client.retrieveEncryptedPackage("invalid-cid"),
    ...
    test("should handle storage failures gracefully", async () => {
      // Mock upload failure
      const pinataInstance = (client as any).pinata;
      pinataInstance.upload.json.mockRejectedValueOnce(
        new Error("Network error"),
      );

      await expect(client.storePackage({ test: "data" })).rejects.toThrow(
        "Failed to store package on IPFS",
      );
    });
  });
