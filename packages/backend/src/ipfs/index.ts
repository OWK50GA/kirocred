...
export class IPFSClient {
   ...
   */
  constructor(pinataJwt: string, encryptionKey?: Buffer) {
  ...
  private decryptPackageData(encryptedPackage: EncryptedPackage): any {
    ...
    let plaintext = decipher.update(encryptedPackage.encryptedData, 'hex', 'utf8');
   ...
   */
  async storePackage(packageData: any): Promise<string> {
    try {
      ...
      throw new Error(`Failed to store package on IPFS: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async retrievePackage(cid: string): Promise<any> {
    try {
      ...
      throw new Error(`Failed to retrieve package from IPFS: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
   ...
   * @returns IPFS CID
   */
  async storeEncryptedPackage(encryptedPackage: EncryptedPackage): Promise<string> {
    try {
      const upload = await (this.pinata.upload as any).json(encryptedPackage);
      return upload.IpfsHash || upload.cid;
    } catch (error) {
      throw new Error(`Failed to store encrypted package on IPFS: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
  ...
  async retrieveEncryptedPackage(cid: string): Promise<EncryptedPackage> {
    try {
      ...
      throw new Error(`Failed to retrieve encrypted package from IPFS: ${error instanceof Error ? error.message : String(error)}`);
    }
  }
}
 ...
 */
export function createIPFSClient(pinataJwt: string, encryptionKey?: Buffer): IPFSClient {
