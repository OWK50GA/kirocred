...
function hashPair(left: string, right: string): string {
  ...
  const hashBuffer = crypto.createHash('sha256').update(Buffer.from(combined, 'hex')).digest();
...
export function getRoot(tree: MerkleTree): string {
  ...
  if (rootLayer.length !== 1) {
    throw new Error('Invalid merkle tree: root layer should have exactly one element');
  }
