...
export function hashAttributes(attributes: Record<string, any>): string {
  ...
  const sortedAttrs = sortedKeys.reduce((acc, key) => {
...
export function encryptKeyToHolder(
  ...
  const ephemeralPublicKeyFull = ec.starkCurve.getPublicKey(ephemeralPrivateKey);
  
  const holderPublicKeyHex = holderPublicKeyFull.slice(2);
  const ephemeralPubkeyHex = Buffer.from(ephemeralPublicKeyFull).toString('hex');

  // Compute shared secret
  let sharedSecret = ec.starkCurve.getSharedSecret(ephemeralPrivateKey, hexToUint8Array(holderPublicKeyFull));
  const derivedKey = crypto.createHash('sha256').update(Buffer.from(sharedSecret)).digest(); // 32 bytes
...
export function decryptKeyFromHolder(
  ...
  const derivedKey = crypto.createHash('sha256').update(Buffer.from(sharedSecret)).digest();
