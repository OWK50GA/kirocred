...
describe('Crypto Module', () => {
  ...
  describe('Commitment Computation', () => {
    test('computeCommitment produces consistent commitment', () => {
      ...
      const commitment1 = computeCommitment(credId, holderPubkey, attrsHash, salt);
      const commitment2 = computeCommitment(credId, holderPubkey, attrsHash, salt);
  ...
  describe('Signature Verification', () => {
    ...
    test('verifyIssuerSignature delegates to verifySignature', () => {
      const privateKey = stark.randomAddress();
      const publicKey = stark.getFullPublicKey(privateKey);
      const messageHash = '0x0123456789abcdef0123456789abcdef0123456789abcdef0123456789abcdef';
