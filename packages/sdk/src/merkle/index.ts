...
function hashPair(left: string, right: string): string {
  ...
  const combined = leftHex + rightHex;
    const hashBuffer = sha256(hexToBytes(combined))
...
export function verifyProof(
  ...
  if (pathElements.length === 0) {
    const cleanLeaf = encode.removeHexPrefix(leaf);
    const truncatedCleanLeaf = cleanLeaf.slice(0, 60);
    return encode.addHexPrefix(truncatedCleanLeaf) === expectedRoot;
  }
  ...
  const cleanCurrentHash = encode.removeHexPrefix(currentHash);
  const truncatedCleanCurrentHash = cleanCurrentHash.slice(0, 60);

  return encode.addHexPrefix(truncatedCleanCurrentHash) === expectedRoot;
}
