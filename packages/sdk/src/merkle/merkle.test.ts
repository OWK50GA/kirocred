...
//   return "0x" + crypto.createHash("sha256").update(data).digest("hex");
    return "0x" + bytesToHex(sha256((utf8ToBytes(data))));
}
...
describe("Merkle Tree Builder", () => {
  describe("buildTree", () => {
    ...
    it("should throw error for empty commitments array", () => {
      expect(() => buildTree([])).toThrow(
        "Cannot build merkle tree from empty commitments array",
      );
    });
  });
  ...
  describe("getRoot", () => {
    it("should return root from single leaf tree", () => {
      ...
      expect(root).toBe(encode.addHexPrefix(encode.removeHexPrefix(commitments[0]).slice(0, 60)));
    });
    ...
    it("should return root from multi-leaf tree", () => {
      ...
      expect(root.length).toBe(62); // 0x + 64 hex chars
    });
  });
